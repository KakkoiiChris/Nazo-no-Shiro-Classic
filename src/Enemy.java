//Christian Alexander, 1/1/11, Pd. 6
    ...
    public abstract class Enemy extends Fighter
   {
       ...
       public void use(Fighter self)
      {
         ...
         if(effectives.size() > 0)
         {
            for(int j=0; j<effectives.size(); j++)
            {
               if(effectives.get(j).getName().equals("Corrupt") && health <= 10)
               {                   
                  ...
                  index = search("Corrupt", effectives);
                  if(index >= 0)
                     effectives.remove(index);
                  else
                     System.out.println("NO!!!!!"); 
               ...
               switch(rand)
               {
                  case 1:
                     for(int k=0; k<useable.size(); k++)
                     {
                        ...
                        {
                           effectives.add(useable.remove(k));
                           done = true;
                           break;
                        }
                     }
