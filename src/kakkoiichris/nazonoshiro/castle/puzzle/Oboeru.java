...
import kakkoiichris.kotoba.util.Convert;
...
public class Oboeru extends Puzzle {
    ...
    public Event doRound(Console console) {
        ...
        while (count < 15) {
            ...
            int row1 = Convert.toInt(console.readLine().orElseThrow()).orElse(-1);

            while (row1 < 0 || row1 > board.length) {
                console.writeLine("That's out of your range. Enter again.");

                row1 = Convert.toInt(console.readLine().orElseThrow()).orElse(-1);
            }

            console.setPrompt("Column 1 > ");

            int column1 = Convert.toInt(console.readLine().orElseThrow()).orElse(-1);

            while (column1 < 0 || column1 > board[0].length) {
                console.writeLine("That's out of your range. Enter again.");

                column1 = Convert.toInt(console.readLine().orElseThrow()).orElse(-1);
            }
            ...
            int row2 = Convert.toInt(console.readLine().orElseThrow()).orElse(-1);

            while (row2 < 0 || row2 > board.length) {
                console.writeLine("That's out of your range. Enter again.");

                row2 = Convert.toInt(console.readLine().orElseThrow()).orElse(-1);
            }

            console.setPrompt("Column 2 > ");

            int column2 = Convert.toInt(console.readLine().orElseThrow()).orElse(-1);

            while (column2 < 0 || column2 > board[0].length) {
                console.writeLine("That's out of your range. Enter again.");

                column2 = Convert.toInt(console.readLine().orElseThrow()).orElse(-1);
            }
